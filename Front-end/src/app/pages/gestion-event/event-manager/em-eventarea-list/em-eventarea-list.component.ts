import {Component, OnInit, ViewChild} from '@angular/core';
    ...
    @Component({
      selector: 'app-em-eventarea-list',
      standalone: true,
      imports: [
        BsDropdownModule,
        DropzoneModule,
      ...
      templateUrl: './em-eventarea-list.component.html',
      styleUrl: './em-eventarea-list.component.scss'
    })
    export class EmEventareaListComponent implements OnInit {
      ...
      ngOnInit(): void {
        ...
        this.breadCrumbItems = [
          ...
          { label: 'My Event Areas', active: true }
        ];
        ...
        setTimeout(() => {
          ...
          // Only load event areas for the current user
      ...
      // Load event areas for the current user
      ...
      selectstatus() {
        ...
        if (status) {
          this.eventAreas = this.allEventAreas.filter((item: any) => { return status == item.status })
        } else {
          this.eventAreas = this.allEventAreas.slice(0, this.itemsPerPage);
        }
      }
      ...
      confirmDelete() {
        this.eventAreaService.deleteEventArea(this.deleteID).subscribe({
          next: () => {
            ...
            this.allEventAreas = this.allEventAreas.filter(area => area.id !== this.deleteID);
            this.refreshPagination();
